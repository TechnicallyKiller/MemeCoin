...
const BugReportForm = (props) => {
    ...
    createEffect(() => {
        const titleValid = isTitleValid(), descriptionValid = isDescriptionValid(), stepsValid = areStepsValid(), userAllow = didUserAllow();
    ...
    const handleSubmit = async (event) => {
        ...
        const request = {
            userId: beaconState[StorageKey.USER_ID] && beaconState[StorageKey.USER_ID].length
    ...
    return (<form onSubmit={handleSubmit} class="form-style">
      <div class="input-group">
        <label for="title" class="label-style">
          Title
        </label>
        <input type="text" id="title" value={title()} onBlur={(e) => {
            !titleTouched() && setTitleTouched(true);
            setTitle(e.currentTarget.value);
        }} class={`input-style ${titleTouched() && titleErrorMsg().length ? 'invalid' : ''}`}/>
        {titleTouched() && titleErrorMsg().length && (<label class="error-label">{titleErrorMsg()}</label>)}
      </div>
      <div class="input-group">
        <label for="description" class="label-style">
          Description
        </label>
        <textarea id="description" value={description()} onBlur={(e) => {
            !descriptionTouched() && setDescriptionTouched(true);
            setDescription(e.currentTarget.value);
        }} class={`textarea-style ${descriptionTouched() && descriptionErrorMsg().length ? 'invalid' : ''}`}/>
        {descriptionTouched() && descriptionErrorMsg().length && (<label class="error-label">{descriptionErrorMsg()}</label>)}
      </div>
      <div class="input-group">
        <label for="steps" class="label-style">
          Steps to Reproduce
        </label>
        <textarea id="steps" value={steps()} onBlur={(e) => {
        ...
        {stepsTouched() && stepsErrorMsg().length && (<label class="error-label">{stepsErrorMsg()}</label>)}
      </div>
      <div class="permissions-group">
        <label for="user-premissions">You agree to share anonymous data with the developers.</label>
        <input id="user-premissions" type="checkbox" onChange={() => setDidUserAllow((prev) => !prev)}/>
      </div>
      <button type="submit" disabled={!isFormValid()} class={`button-style ${isFormValid() ? 'valid' : 'invalid'} ${isLoading() ? 'button-loading' : ''}`}>
        {!isLoading() && !status() ? 'Submit' : <>&nbsp;</>}
        {!isLoading() && status() && (<span class={status() === 'success' ? 'icon success-icon' : 'icon error-icon'}>
            {status() === 'success' ? '✓' : '✕'}
          </span>)}
        {showThankYou() && (<div class="thank-you-message">
            <For each={'Thank You!'.split('')}>
              {(letter, index) => (<span style={{ 'animation-delay': `${index() * 0.1}s` }}>{letter}</span>)}
            </For>
          </div>)}
      </button>
    </form>);
