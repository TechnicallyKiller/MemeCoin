    ...
    if (!isOpen()) {
        ...
        const generateWCError = (title) => {
          ...
          <h4>
            ...
            <span style={{ 'text-decoration': 'underline', color: '#007bff', cursor: 'pointer' }} onClick={() => setCurrentInfo('help')}>
              here
            </span>
          </h4>
        ...
        const QRCode = ({ isMobile }) => {
            ...
            return (<>
          {isConnected ? (<QR isWalletConnect={currentWallet()?.supportedInteractionStandards?.includes('wallet_connect') || false} isMobile={isMobile} walletName={currentWallet()?.name || 'AirGap'} code={codeQR()} onClickLearnMore={handleClickLearnMore} onClickQrCode={handleClickQrCode}/>) : (generateWCError(`Connect with ${currentWallet()?.name} Mobile`))}
        </>);
        };
        ...
        dispose = render(() => (<div class={`theme__${colorMode}`}>
          ...
          {config.pairingPayload && (<Alert loading={isLoading()} open={isOpen()} showMore={showMoreContent()} content={<div>
                  {currentInfo() === 'install' && (<div style={currentInfo() === 'install' || currentInfo() === 'qr'
                      ...
                      {isOnline && currentWallet()?.types.includes('web') && (<Info border title={`Connect with ${currentWallet()?.name} Web`} description={`(It will open the wallet in a new tab)`} buttons={[
                                {
                                    label: 'Use Browser',
                                    type: 'primary',
                      ...
                      {!isMobile() && currentWallet()?.types.includes('desktop') && (<Info border title={`Connect with ${currentWallet()?.name} Desktop App`} description={`If you don't have the desktop app installed, click below to download it.`} buttons={[
                                {
                      ...
                      {!isMobile() &&
                            ...
                            currentWallet()?.types.length > 1 && (<QRCode isMobile={false}/>)}
                      {!isMobile() &&
                            codeQR().length > 0 &&
                            currentWallet()?.types.includes('ios') &&
                            currentWallet()?.types.length <= 1 && (<QRCode isMobile={true}/>)}
                      {isMobile() &&
                            currentWallet()?.types.includes('ios') &&
                            (!currentWallet()?.supportedInteractionStandards?.includes('wallet_connect') || isWCWorking() ? (<Info border title={`Connect with ${currentWallet()?.name} Mobile`} description={''} buttons={[
                                    {
                                        label: 'Use App',
                                        type: 'primary',
                                        onClick: async () => {
                                ...
                                ]} downloadLink={currentWallet()?.name.toLowerCase().includes('kukai') && isIOS(window)
                                    ? {
                                        label: 'Get Kukai Mobile >',
                                        url: 'https://ios.kukai.app'
                                    }
                                    : undefined} onShowQRCodeClick={async () => {
                                    const syncCode = currentWallet()?.supportedInteractionStandards?.includes('wallet_connect')
                                        ? (await wcPayload)?.uri ?? ''
                                        : await new Serializer().serialize(await p2pPayload);
                                    const wallet = currentWallet();
                  ...
                  {currentInfo() === 'qr' && (<div style={currentInfo() === 'install' || currentInfo() === 'qr'
                      ...
                      {!displayQrExtra() ? (<PairOther walletList={walletList()} onClickLearnMore={handleClickLearnMore} p2pPayload={p2pPayload} wcPayload={wcPayload}></PairOther>) : (<QRCode isMobile={true}/>)}
                    </div>)}
                  ...
                  <div style={currentInfo() === 'help'
                    ...
                    {areMetricsEnabled && (<BugReportForm onSubmit={() => {
                            handleCloseAlert();
                        }}/>)}
                    {!areMetricsEnabled && (<>
                ...
                </div>} extraContent={currentInfo() !== 'top-wallets' || isMobile() ? undefined : (<Wallets small wallets={walletList().slice(-(walletList().length - 4))} isMobile={isMobile()} onClickWallet={handleClickWallet} onClickOther={handleClickOther}/>)} onClickShowMore={handleClickShowMoreContent} onCloseClick={() => handleCloseAlert()} onBackClick={currentInfo() === 'install'
                    ? () => setCurrentInfo('top-wallets')
                    : currentInfo() === 'qr'
                        ? () => setCurrentInfo('top-wallets')
